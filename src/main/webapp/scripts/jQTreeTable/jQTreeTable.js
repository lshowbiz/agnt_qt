...
(function($){
	...
	$.fn.jqTreeTable = function(map, options){
		...
		$("tr", this).each(function(i){							//Inject the images into the column to make it work
			$(this).children("td").eq(opts.column).prepend(mapb[i]);
		});
		...
		if(opts.striped){$("#treetable tr:even").addClass("even")};//Stripe all even rows

		$(".parimg", this).each(function(i){
			var $this = $(this);
			$this.click(function(){
		...
		$("#treetable").click( function( event ) {					//This looks after selecting td's
			var $target = (event.target.tagName == "TD")?$(event.target):$(event.target).parents("td");
			if (typeof opts.onselect == "function"){
				$("td.selected", this).removeClass("selected");
	...
	expandKids = function(num){											//Expands immediate children
		...
		for (var x=0, xl=mapa[num].length;x<xl;x++){
			$("#treetable tr").eq(mapa[num][x]-1).removeClass("collapsed");
		}
	}
	...
	collapseKids = function(num){										//Recursively collapses all children and their children and changes
		...
		for (var x=0, xl=mapa[num].length;x<xl;x++){
			mapnumx = mapa[num][x];
			$("#treetable tr").eq(mapnumx-1).addClass("collapsed");
			if (mapa[mapnumx]){
	...
	stripe = function(num){												//Dynamically stripes rows after expanding/collapsing
		var isStriped = ($("#treetable tr").eq(num).attr("class")=="even over") ? 0 : 1;
		$("#treetable tr:gt(num)").not(".collapsed").each(function(i){
			if ((i+isStriped) % 2==0){
