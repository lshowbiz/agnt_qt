...
<p><ng:locale key="miRecommendRef.total"/>:${empty jmiRecommendedInquiryListCount?0:jmiRecommendedInquiryListCount }</p>
<div id="kkk"></div>
...
tree.setIconPath("images/treeimages/");
	<c:forEach items="${jmiRecommendedInquiryList}" var="mi" varStatus="status">
	...
	<c:if test="${mi.jmiMember.cardType=='4' }">
		...
		<c:if test="${status.count == 1}">
			...
			tree.N["0_<c:out value="${mi.userCode}"/>"] = "T:<font color=#0066CC><c:out value="${mi.userCode} - ${mi.jmiMember.petName}"/></font> [ <font color='${cardType}'><ng:code listCode="bd.cardtype" value="${mi.jmiMember.cardType}"/></font> ] <font color=#FF0000><ng:locale key="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0?'login.Out':'' }"/><fmt:formatDate value="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0 ? mi.jmiMember.exitDate : '' }" pattern="yyyy-MM-dd"/></font>[<ng:locale key="${(not empty mi.jmiMember.checkDate)?('logType.C'):('pd.createTime') }"/>:<fmt:formatDate value="${(not empty mi.jmiMember.checkDate)?(mi.jmiMember.checkDate):(mi.jmiMember.createTime) }" pattern="yyyy-MM-dd"/>] <ng:code listCode="store.type" value="${mi.jmiMember.isstore }"/>   <font color=#FF0000><ng:locale key="${mi.jmiMember.active=='1' ? 'is.active' : '' }"/></font>   <font color=#FF0000><ng:locale key="${mi.jmiMember.freezeStatus=='1' ? 'miMember.freezestatus1' : '' }"/></font>;";
		</c:if>
		<c:if test="${status.count != 1}">
			tree.N["<c:out value="${mi.jmiMember.recommendNo}"/>_<c:out value="${mi.userCode}"/>"] = "T:<font color=#0066CC><c:out value="${mi.userCode} - ${mi.jmiMember.petName}"/></font> [ <font color='${cardType}'><ng:code listCode="bd.cardtype" value="${mi.jmiMember.cardType}"/></font> ] - <font color=#FF0000><fmt:formatNumber value="${(fn:length(mi.treeIndex)-rootTreeIndex)/3}" pattern="##"/></font> <font color=#FF0000><ng:locale key="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0?'login.Out':'' }"/><fmt:formatDate value="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0 ? mi.jmiMember.exitDate : '' }" pattern="yyyy-MM-dd"/></font>[<ng:locale key="${(not empty mi.jmiMember.checkDate)?('logType.C'):('pd.createTime') }"/>:<fmt:formatDate value="${(not empty mi.jmiMember.checkDate)?(mi.jmiMember.checkDate):(mi.jmiMember.createTime) }" pattern="yyyy-MM-dd"/>] <ng:code listCode="store.type" value="${mi.jmiMember.isstore }"/>   <font color=#FF0000><ng:locale key="${mi.jmiMember.active=='1' ? 'is.active' : '' }"/></font>   <font color=#FF0000><ng:locale key="${mi.jmiMember.freezeStatus=='1' ? 'miMember.freezestatus1' : '' }"/></font>;";
		</c:if>
	</c:forEach>
