...
<%@ include file="/common/taglibs.jsp"%>
	...
	<script type="text/javascript" src="scripts/jQTreeTable/jQTreeTable.js"></script>
...
<div class="cont">	
    ...
    <form method="get" action="jbdBonusTreeSelect" target="blank" onsubmit="return validateOthers(this)" id="bdPeriodForm">
			<table class="search_table mt"  >
                    <tr>
                        ...
                        <td>期别（例：200806）:</td>
                        <td><input type="text" name="formatedWeek" id="formatedWeek" size="8" value="${not empty param.formatedWeek ? param.formatedWeek:curBdPeriod }"/></td>
                        <td>
...
<%--<p>
    ...
    <a href="jbdBonusTree?strAction=bdBonusTreeQuery&formatedWeek=${param.formatedWeek }&userCode=${sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal.userCode }">
    ${sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal.userName }
    </a>
    <c:if test="${not empty upLinkList}">
    <c:forEach items="${upLinkList}" var="upMiLinkRefVar">
    / <a href="jbdBonusTree.html?strAction=bdBonusTreeQuery&formatedWeek=${param.formatedWeek }&userCode=${upMiLinkRefVar.user_code }">
...
<table id="tablemain" width="100%" border="1" class="tabQueryLs">
	<thead>
		<tr>
			...
			<th><ng:locale key="miLinkRef.total"/></th>
			<th><ng:locale key="bdBonus.tree.personPv"/></th>
			<th><ng:locale key="bdday.pending_recommend_pv2"/></th>
			<th><ng:locale key="bdBonus.tree.groupPv"/></th>
			<th><ng:locale key="bdBonus.month_group_pv"/></th>
			<th><ng:locale key="bdday.pending_pv"/></th>
			<th><ng:locale key="bd.month.area.total.pv"/></th>
		    <th><ng:locale key="jbdMemberLinkCalcHist.areaConsumption"/></th>
	...
	<tbody id="treetable">
		<c:forEach items="${jbdDayBonusList}" var="jbdDayBonusVar" varStatus="jbdDayBonusVarStatus">
			<tr>
				...
				<td><img src="images/bonus_tree/tree_person_pv.gif" border="0"/> <fmt:formatNumber pattern="###,##0.00">${jbdDayBonusVar.month_group_pv }</fmt:formatNumber></td>
				...
				<td><a href="jbdBonusTree.html?strAction=bdBonusTreeQuery&formatedWeek=${param.formatedWeek }&userCode=${jbdDayBonusVar.user_code }"><img src="images/bonus_tree/next_5.png" border="0"/></a></td>
