...
<div class="cont">	
	...
	<div class="mt">
	<form:form commandName="linkman" method="post"
		action="jadLinkmanDetailUpdate" id="jadLinkmanDetailUpdateId"
			...
			<tbody>
				<tr>
					<td class="tr">
						...
						<ng:locale key="linkman.name" />：
					</td>
					<td>
						<form:input path="name" id="name" cssClass="text medium" size="20"
							maxlength="100" />
					</td>
					<td class="tr">
						<span class="star"></span>
					...
					<td>
						<ng:list name="sex" listCode="sex" value="${linkman.sex}"
							defaultValue="" />
					</td>
				</tr>
				<tr>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="linkman.birthday" />：
						</span>
					</td>
					<td>
					...
					<td class="tr">
						<span class="star"></span>
						<ng:locale key="linkman.mobilephone" />：
					</td>
					<td>
						<form:input path="mobilephone" id="mobilephone"
							cssClass="text medium" size="20" maxlength="20" />
					</td>
				</tr>
				<tr>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="linkman.nextContactTime" />：
						</span>
					</td>
					<td>
					...
					<td>
						<ng:list name="customerSource" listCode="linkman.customersource"
							value="${linkman.customerSource}" defaultValue="" />
					</td>
				</tr>
				<tr>
					<td class="tr">
					...
					<td>
						<form:select path="lcId">
							<form:option value="">请选择</form:option>
							<form:options items="${linkmanCategoryList}" itemValue="id"
								itemLabel="name" />
						</form:select>
					</td>
					<td class="tr">
						<span for="" class="pdl10">
							客户类型：
						</span>
					</td>
					<td>
						<ng:list name="linkmanStatus" listCode="linkman.status"
							value="${linkman.linkmanStatus}"
							defaultValue="" />
					</td>
				</tr>
				<!-- 家庭住址信息 -->
				<tr>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="linkman.familyPhone" />
							：
						</span>
					</td>
					<td>
						<form:input path="familyPhone" id="familyPhone"
							cssClass="text medium" size="25" maxlength="20" />
					</td>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="linkman.familyDetailAddress" />
							：
						</span>
					</td>
					<td>
						<form:input path="familyAddress" id="familyAddress"
							cssClass="text medium" size="85" maxlength="200" />
					</td>
				</tr>
				<tr>
					<td class="tr">
					...
					<td colspan="3">
						<form:select path="province"
							onchange="getCity();">
							<form:option value="">请选择</form:option>
							<form:options items="${alStateProvinces}"
								itemValue="stateProvinceId" itemLabel="stateProvinceName" />
						</form:select>
						&nbsp;
				...
				<tr>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="miMember.companyName" />：
						</span>
					</td>
					<td>
						<form:input path="company" id="company" cssClass="text medium"
							size="85" maxlength="100" />
					</td>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="miMember.companyAddr" />：
						</span>
					</td>
					<td>
				...
				<tr>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="linkman.companyPhone" />：
						</span>
					</td>
					<td>
					...
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="miMember.faxtele" />：
						</span>
					</td>
					<td>
						<form:input path="fax" id="fax" cssClass="text medium" size="25"
							maxlength="20" />
					</td>
				</tr>
				<tr>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="subStore.postalcode" />：
						</span>
					</td>
					<td>
						<form:input path="post" id="post" cssClass="text medium" size="25"
							maxlength="6" onkeyup="this.value=this.value.replace(/\D/g,'')" />
					</td>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="micro.channel" />：
						</span>
					</td>
					<td>
						<form:input path="microMessage" id="microMessage"
							cssClass="text medium" size="25" maxlength="11" />
					</td>
				</tr>
				<!--客户其他信息 -->
				<tr>
					...
					<td>
						<form:input path="qq" id="qq" cssClass="text medium" size="25"
							maxlength="11" />
					</td>
					<td class="tr">
						<span class="pdl10">
							<ng:locale key="linkman.weibo" />：
						</span>
					</td>
					<td>
						<form:input path="weibo" id="weibo" cssClass="text medium"
				...
				<tr>
					...
					<td>
						<form:input path="msn" id="msn" cssClass="text medium" size="25"
							maxlength="32" />
					</td>
					<td class="tr">
						<span for="" class="pdl10">
							<ng:locale key="subStore.email" />：
						</span>
					</td>
					<td>
				...
				<tr>
					...
					<td colspan="3">
						<form:textarea path="remark" id="remark" cssClass="text medium"
							style="width:99%;height:70px;margin-top:8px;" resize="none"
							cssErrorClass="text medium error"
							onkeydown="checkNumber(this.value)" />
					</td>
				</tr>
				<!-- 定义隐藏域,让ID和userCode的值也传到后台，这样方便做修改操作，否则的话，是做新增操作 -->
				<form:input path="id" id="id" cssClass="text medium" />
				<form:input path="userCode" id="userCode" cssClass="text medium" />
			</tbody>
		</table>
		<div class="tc" >
			<button type="submit" name="save" id="saveId" class="btn btn-info"><ng:locale key="operation.button.save"/></button>
			<button type="button" class="btn btn-success" name="cancel" onclick="returnOld()"><ng:locale key="operation.button.cancel"/></button>
		</div>
		</div>
	</form:form>
</div>	
	<v:javascript formName="linkman" cdata="false" dynamicJavascript="true"
