package com.joymain.ng.webapp.controller;

import com.joymain.ng.dao.SearchException;
import com.joymain.ng.service.FiBcoinPayconfigDetailManager;
import com.joymain.ng.model.FiBcoinPayconfigDetail;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ExtendedModelMap;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;

@Controller
@RequestMapping("/fiBcoinPayconfigDetails/")
public class FiBcoinPayconfigDetailController {
    private FiBcoinPayconfigDetailManager fiBcoinPayconfigDetailManager;

    @Autowired
    public void setFiBcoinPayconfigDetailManager(FiBcoinPayconfigDetailManager fiBcoinPayconfigDetailManager) {
        this.fiBcoinPayconfigDetailManager = fiBcoinPayconfigDetailManager;
    }

    @RequestMapping(method = RequestMethod.GET)
    public Model handleRequest(@RequestParam(required = false, value = "q") String query)
    throws Exception {
        Model model = new ExtendedModelMap();
        try {
            model.addAttribute(fiBcoinPayconfigDetailManager.search(query, FiBcoinPayconfigDetail.class));
        } catch (SearchException se) {
            model.addAttribute("searchError", se.getMessage());
            model.addAttribute(fiBcoinPayconfigDetailManager.getAll());
        }
        return model;
    }
    
}
