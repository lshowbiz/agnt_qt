 ...
 */
@Controller
@RequestMapping("mobilePpPay")
@SuppressWarnings("rawtypes")
public class MobilePayByFiProductPointController extends BaseFormController {
	
	private Log log = LogFactory.getLog(MobilePayByFiProductPointController.class);
	 ...
	 * @return 交易流水
	 */
    @RequestMapping(value="api/getPpJournals")
	@ResponseBody
	public List getFiProductPointJournalListByUserCode(String userId, String token,String dealStartTime, String dealEndTime,int pageNum,int pageSize){
		...
		//查询交易流水
    	List<FiProductPointJournal> fiProductPointJournalList=fiProductPointJournalManager.getFiProductPointJournalListByUser(userId,dealStartTime,dealEndTime,pageNum,pageSize);

    	return fiProductPointJournalList;
	}
     ...
     * @return 产品积分帐户余额
     */
    @RequestMapping(value="api/getPpBalance")
	@ResponseBody
	public String getFiProductPointBalanceManagerByUserCode(String userId,String token){
    	 JsysUser user = jsysUserManager.getUserByToken(userId, token);
    	 Object object = jsysUserManager.getAuthErrorCode(user, "String");
 		 if(null!=object){
 		 	return (String)object;
 		 }
    	FiProductPointBalance fiProductPointBalance=fiProductPointBalanceManager.getProductPointBalance(userId, "1");
    	return fiProductPointBalance.getBalance().toString();
    }
     ...
     * @param userId 用户编号
     * @param orderId  订单id
     * @param userNum  使用产品积分数
     * @return 返回码：1代表支付成功, 0代表支付失败
     */
    @RequestMapping(value="api/payOrderByPp")
	@ResponseBody
    public String payJpoMemberOrderByFiProductPoint(String userId, String orderId, String userNum, String token,String payType){
    	JsysUser user = jsysUserManager.getUserByToken(userId, token);
    	Object object = jsysUserManager.getAuthErrorCode(user, "String");
    	...
    	if(amount.compareTo(new BigDecimal("0"))==1){
    		...
    		//判断,设置
    		if(amount.compareTo(jpoMemberOrder.getAmount())>=0){
    			
    			//使用产品积分数>=订单金额，全部用基金支付
    			order.setProductPointAmount(jpoMemberOrder.getAmount());
//    			order.setAmount(new BigDecimal("0"));
        	}else{
        		
        		//使用产品积分数<订单金额，其余部分用电子存折支付
...
//        		order.setAmount(jpoMemberOrder.getAmount().subtract(amount));
    	...
    	try{
			...
			if("1".equals(order.getPayByProduct())){
				log.info("MobilePayByFiProductPointController start:"+order.getMemberOrderNo());
				jpoMemberOrderManager.checkJpoMemberOrderByProductPoint(order, user,"2");
				log.info("MobilePayByFiProductPointController stop:"+order.getMemberOrderNo());
				//Modify By WUCF 20160805 新增订单支付来源标示字段：PAYMENT_TYPE修改 
				jpoMemberOrderManager.updateJpoMemberOrderPaymentType(String.valueOf(jpoMemberOrder.getMoId()), "17");
				...
				log.info("MobilePayByFiProductPointController stop:"+order.getMemberOrderNo());
				//Modify By WUCF 20160805 新增订单支付来源标示字段：PAYMENT_TYPE修改
				jpoMemberOrderManager.updateJpoMemberOrderPaymentType(String.valueOf(jpoMemberOrder.getMoId()), "16");
				log.info("updateJpoMemberOrderPaymentType stop:"+order.getMemberOrderNo());
			}
			b = true;
...
//			log.info("call function END. ");
		...
		if(b){
			try {
				...
				log.info("=======审单MQ：MobilePayByFiProductPointController orderNo is:"+jpoMemberOrder.getMemberOrderNo());
			} catch (Exception e) {
